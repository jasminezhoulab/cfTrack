...
def find_location_on_read(mapping_position, query_position, CIGAR_string, max_length):
	...
	for i in range(0, len(cigar_split) - 1):
		...
		if cigar_case in BOTH_ADD_CIGAR:
			...
			if read_current_position >= max_length and query_position > reference_current_position:
			...
			if reference_current_position >= query_position:
				if read_current_position < reference_current_position - query_position:
					...
					if position_on_read >= max_length :
						return "NA"
					else:
			...
			read_current_position += num_bases
			if read_current_position >= max_length :
...
reference_base_dict = defaultdict(list)
...
with open(sys.argv[1], 'r') as BED:
	for line in BED:
		...
		reference_base_dict[chr].append(ref)  ### special for RANDOM
...
#						related_position.append(var_loc_on_read1)
						...
						if basestring1[var_loc_on_read1] != reference_base_dict[chr1][i_var]:
							related_variant.append(i_var)
							related_read.append(1)
							related_position.append(var_loc_on_read1)
							continue
...
#						related_position.append(var_loc_on_read0)
						continue
					elif basestring0[var_loc_on_read0] != reference_base_dict[chr0][i_var]:
						related_variant.append(i_var)
						related_read.append(0)
						related_position.append(var_loc_on_read0)
			...
			for i_rel_var in range(len(related_variant)):
				...
				if related_read[i_rel_var] == 0:
					...
					out_list = INFOA_0 + RELATIONA + INFOA_1 + EXTRA + SUP_INFO_0 + SUP_INFO_1 + VARIANT + [related_position[i_rel_var], len(basestring0) - related_position[i_rel_var], CIGAR0.find('S'), readname0]
					...
					out_list = INFOA_1 + RELATIONA + INFOA_0 + EXTRA + SUP_INFO_1 + SUP_INFO_0 + VARIANT + [related_position[i_rel_var], len(basestring1) - related_position[i_rel_var], CIGAR1.find('S'), readname1]
					out_list = [str(i) for i in out_list]
...
#						related_position1.append(var_loc_on_read1)
						continue
					elif basestring1[var_loc_on_read1] != reference_base_dict[chr1][i_var]:
						related_variant1.append(i_var)
						related_position1.append(var_loc_on_read1)
...
#						related_position0.append(var_loc_on_read0)
			...
			for i_rel_var in range(len(related_variant1)):
				...
				INFOA_0 = INFO_0 + quality0 + base0 + cigar0 + [indel0, homopolymer0]
				RELATIONA = RELATION + [strand1, strand0, flag1, flag0]
				out_list = INFOA_1 + RELATIONA + INFOA_0 + EXTRA + SUP_INFO_1 + SUP_INFO_0 + VARIANT + [related_position1[i_rel_var], len(basestring1) - related_position1[i_rel_var], CIGAR1.find('S'), readname1]
				out_list = [str(i) for i in out_list]
				if CIGAR1.find('S') > 0 or CIGAR1.find('H') > 0:
					continue
				OUTPUT.write('\t'.join(out_list) + '\n')
			for i_rel_var in range(len(related_variant0)):
				...
				INFOA_0 = INFO_0 + quality0 + base0 + cigar0 + [indel0, homopolymer0]
				RELATIONA = RELATION + [strand0, strand1, flag0, flag1]
				out_list = INFOA_0 + RELATIONA + INFOA_1 + EXTRA + SUP_INFO_0 + SUP_INFO_1 + VARIANT + [related_position0[i_rel_var], len(basestring0) - related_position0[i_rel_var], CIGAR0.find('S'), readname0]
				out_list = [str(i) for i in out_list]
				if CIGAR0.find('S') > 0 or CIGAR0.find('H') > 0:
					continue
				OUTPUT.write('\t'.join(out_list) + '\n')
