...
# input parameter
input_list=demo/input_list.calculate_IVAF_random_marker.txt
...
${java} -Xmx10g -jar ${picard} BedToIntervalList I=${bed} O=${pathname}.interval_list SD=${reference_dict}
...
${python} ${source_path}/py6.machinelearn.bed_for_getfasta.random.py ${window} ${bed} ${pathname}.getfastabed
${bedtools} getfasta -bedOut -fi ${reference} -bed ${pathname}.getfastabed > ${pathname}.fastabed
python ${source_path}/py7.machinelearn.bed_for_feature.random.py ${window} ${pathname}.fastabed ${pathname}.preparebed

### EXTRACT FEATURES FROM READS

${python} ${source_path}/py8.machinelearn.extract_features_from_reads_filter_cluster_RANDOM.with_readname.py ${pathname}.preparebed ${pathname}.paired-reads.qsort.sam ${pathname}.paired-reads.qsort.features ${pathname}.filter_cluster.bed
...
${python} ${source_path}/py12.merge_output.add_readname.py ${pathname}.paired-reads.qsort.overlap.features.expand ${pathname}.paired-reads.qsort.nonoverlap.features.expand ${pathname}.paired-reads.qsort.overlap.features.expand.RFpred.csv ${pathname}.paired-reads.qsort.nonoverlap.features.expand.RFpred.csv ${pathname}.paired-reads.qsort.overlap.features ${pathname}.paired-reads.qsort.nonoverlap.features ${pathname}.overlap.merged_output ${pathname}.nonoverlap.merged_output
